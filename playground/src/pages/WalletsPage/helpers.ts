      ...
      setModalState({
        ...
        props: {
          onSubmit: (name: string, password: string) =>
            withLoader(async () => {
              try {
                await sdk.deriveAccount(walletId, name, password);
                closeModal();
              } catch (error) {
                console.error(error);
                alert((error as Error)?.message ?? "Failed to derive account");
              }
            }),
          onClose: closeModal,
        },
            ...
            withLoader(async () => {
              try {
                downloadTextFile(
                  ...
                  await sdk.exportWalletKeyfile(walletId, password),
                  "application/json",
                );
                closeModal();
              } catch (error) {
    ...
    removeWallet: (walletId: string) =>
      withLoader(async () => {
        ...
        try {
          ...
          console.error(error);
          alert((error as Error)?.message ?? "Failed to remove wallet");
        }
      }),
    ...
    renameAccount: (walletId: string, accountId: string) => {
      ...
      withLoader(async () => {
        try {
          ...
          console.error(error);
          alert((error as Error)?.message ?? "Failed to rename account");
        }
      });
    },
    ...
    removeAccount: (walletId: string, accountId: string) =>
      withLoader(async () => {
        ...
        try {
          ...
          console.error(error);
          alert((error as Error)?.message ?? "Failed to remove account");
        }
      }),
  };
};
