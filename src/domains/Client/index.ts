...
export default class Client {
  ...
  public async createHDWallet(
    ...
    if (!MnemonicService.isMnemonicValid(normalizedMnemonic)) {
      throw new Error("Client.createHDWallet: recovery mnemonic is invalid");
    }
    ...
    if (this.flags?.withInsensitiveCacheStorage) {
      InsensitiveCacheStorageManager.save(
        InsensitiveCacheStorageSerializer.serialize(wallet.getActiveAccount()!),
      );
    }
  ...
  private async ensureSession(
    ...
    if (!passwordProvider || !this.shouldHoldSession() || wallet.isUnlocked()) {
      return;
    }
  ...
  public async unlockWallet(
    ...
    if (
      ...
      throw new Error("Client.unlockWallet: This wallet already unlocked");
    }
  ...
  public async deriveAccount(
    ...
    if (this.flags?.withInsensitiveCacheStorage) {
      InsensitiveCacheStorageManager.save(
        InsensitiveCacheStorageSerializer.serialize(createdAccountData.account),
      );
    }
  ...
  public async importWalletKeyfile(
    ...
    const wallet: Wallet = await this.walletManager.importKeyfile(
      await ImportKeyfileService.toImportPayload(keyfile, passwordProvider),
      passwordProvider,
    );
  ...
  public async getBalance(address: Address): Promise<bigint> {
    const balance = await ApiServiceRegistry.getInstance().assets.getBalance(
      address,
      DEFAULT_ASSET,
    );
  ...
  public async getReservations(
    ...
    if (!reservationAdapter) {
      throw new Error("Client.getReservations: Not found reservation adapter");
    }
  ...
  public exploreDeploy(rholang: string): Promise<unknown> {
    return ApiServiceRegistry.getInstance().deploy.exploreDeployData(rholang);
  }
  ...
  private getAccount(wallet: Wallet, accountId: string): Account {
    const account: Account | undefined = wallet.getAccountsMap().get(accountId);
