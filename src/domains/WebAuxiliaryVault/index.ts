...
import { IAuxiliaryVault } from "./IAuxiliaryVault";
...
export class WebAuxiliaryVault implements IAuxiliaryVault {
    ...
    public save(): void {
        ...
        if (!this.encryptedVaultData) {
            throw new Error(
                "Cannot save auxiliary vault without encrypted data",
            );
        }
    ...
    public async unlock(password: string): Promise<void> {
        ...
        if (!this.encryptedVaultData) {
            throw new Error(
                "Auxiliary vault was locked without encrypted data",
            );
        }
        ...
        this.transactions = this.deserializeParsedTransactions(
    ...
    private serializeTransactions(): StoredTransactions {
        return Array.from(this.transactions.entries()).map(
            ([id, transaction]) => [
                id,
                {
                    ...transaction,
    ...
    private deserializeParsedTransactions(
        ...
        return new Map(
            (parsedTransactions as StoredTransactions).map(
                ([id, transaction]) => [
                    id,
                    {
                        ...transaction,
