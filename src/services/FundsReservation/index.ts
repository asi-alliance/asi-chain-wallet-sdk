...
export default class FundsReservationService {
    ...
    public constructor(private transactionsHistory: TransactionsHistory) {}
    ...
    static getInstance(): FundsReservationService {
        if (!FundsReservationService.instance) {
            FundsReservationService.instance = new FundsReservationService(
                null as unknown as TransactionsHistory,
            );
        }
    ...
    lock(
        ...
        if (amount <= 0n) {
            throw new Error(
                "FundsReservationService.lock: Amount must be greater than zero",
            );
        }
    ...
    release(reservationId: string): void {
        ...
        try {
            ...
            throw new Error(
                `FundsReservationService.release: ${error.message}`,
            );
        }
    }
    ...
    finalize(reservationId: string): void {
        ...
        try {
            ...
            throw new Error(
                `FundsReservationService.finalize: ${error.message}`,
            );
        }
    }
     ...
     */
    public async getReservationsByTxs(
        network: Network,
        address: Address,
