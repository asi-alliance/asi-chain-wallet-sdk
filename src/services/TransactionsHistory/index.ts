...
import { Network, NetworkName } from "@domains/aggregates/Network";
...
export class TransactionsHistory {
    ...
    private get auxiliaryVault() {
        if (!this._auxiliaryVault) {
            throw new Error(
                "TransactionsHistory: To call this method, you need to provide auxiliaryVault when initializing the sdk",
            );
        }
    ...
    private get fileSaver() {
        if (!this._fileSaver) {
            throw new Error(
                "TransactionsHistory: To call this method, you need to provide fileSaver when initializing the sdk",
            );
        }
     ...
     */
    public async storeTxInAuxVault(
        deployId: string,
        network: Network,
        amount: bigint,
        fromAddress: string,
        ...
        const allLocalTxs = Array.from(
            this.auxiliaryVault.transactions.values(),
        );
     ...
     */
    private async removeExcessTxsInAuxVault(
    ...
    private async getIndexerTxs(
        ...
        try {
            const txs =
                await BlockchainGateway.getInstance().fetchTransactionHistory(
                    address,
                    pagination,
                );
            return txs;
     ...
     */
    filterAndMapAuxVaultTxsWithAddress(
        auxVaultTxs: Transaction[],
        address: Address,
        networkName: NetworkName,
    ) {
        const filtered = auxVaultTxs.filter(
            (tx) =>
                (tx.from === address || tx.to === address) &&
                tx.networkName === networkName,
        );
        filtered.forEach((tx) => {
            if (tx.to === address) {
    ...
    private sortTxsByTimestamp(
        txs: Transaction[],
        order: Order = "desc",
    ): Transaction[] {
        const sign = order === "asc" ? 1 : -1;
        return txs.sort(
            (a, b) => sign * (a.timestamp.valueOf() - b.timestamp.valueOf()),
        );
    }
    /**
    ...
    private async syncTransactions(
        ...
        const auxVaultTxs = await this.removeExcessTxsInAuxVault(
            indexerTxs,
            auxVaultPassword,
        );
     ...
     */
    public filterLocalTxs(
        allLocalTxs: Transaction[],
        address: Address,
        filter: TransactionFilter | null,
        network: Network,
    ) {
        const filteredByAddressLocalTxs =
            this.filterAndMapAuxVaultTxsWithAddress(
                allLocalTxs,
                address,
                network.name,
            );
        const filteredLocalTxs = this.sortTxsByTimestamp(
            applyTransactionFilter(filteredByAddressLocalTxs, filter, network),
        );
        return filteredLocalTxs;
    }
    public async getAddressOnNetworkLocalTxs(
        network: Network,
        address: Address,
        auxVaultPassword: string,
    ): Promise<Transaction[]> {
        const allLocalTxs = await this.getAllLocalTxs(auxVaultPassword);
        return this.filterAndMapAuxVaultTxsWithAddress(
            allLocalTxs,
            address,
            network.name,
        );
    ...
    public async getFilteredTxs(
        ...
        const filteredLocalTxs = this.filterLocalTxs(
            allLocalTxs,
            address,
            filter,
            network,
        );
        const filteredIndexerTxs = this.sortTxsByTimestamp(
            applyTransactionFilter(indexerTxs, filter, network),
        );
    ...
    public async getMixedTxs(
        ...
        const { localTxs, indexerTxs } = await this.getFilteredTxs(
            network,
            address,
            filter,
            pagination,
    ...
    public async exportTransactions(
        ...
        const transactions = await this.getMixedTxs(
            network,
            address,
            filter,
            pagination,
    ...
    public async downloadTransactions(
        ...
        const data = await this.exportTransactions(
            network,
            address,
            pagination,
            filter,
